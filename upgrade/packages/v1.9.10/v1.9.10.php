...
class up_v1_9_10
{
	...
	public function update_database_optionally()
	{
		...
		$sql = 'ALTER TABLE ' . $ecs->table('category') . ' ADD `touch_icon` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `commission_rate` ;';
		$db->query($sql);
		$sql = 'ALTER TABLE ' . $ecs->table('merchants_category') . ' ADD `touch_icon` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `add_titme`;';
		$db->query($sql);
	}
