...
class up_v1_9_9
{
	...
	public function update_database_optionally()
	{
		...
		$sql = 'UPDATE ' . $ecs->table('admin_action') . ' SET `seller_show` = \'1\' WHERE `action_id` = 47;';
		$db->query($sql);
		$sql = 'INSERT INTO ' . $ecs->table('admin_action') . ' ( `parent_id` , `action_code` , `seller_show` ) VALUES ( 4, \'privilege_seller\', 1 );';
		$db->query($sql);
	}
