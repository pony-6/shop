...
if ($_REQUEST['act'] == 'list') {
	...
	if (!empty($sc_guid) && !empty($sc_rand) && isset($_COOKIE['seller_account_cookie'])) {
		...
		if ($money_status == 1 && $seller_info['seller_money'] < abs($frozen_money) && 0 < $add_sub_frozen_money) {
			sys_msg($_LANG['money_status_prompt_two']);
		}
		else {
			if ($money_status == 1 && $seller_info['frozen_money'] < abs($frozen_money) && $add_sub_frozen_money < 0) {
