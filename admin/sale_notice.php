...
if ($_REQUEST['act'] == 'view') {
	...
	$smarty->assign('ur_here', $_LANG['sale_notice']);
	$smarty->assign('action_link', array('text' => $_LANG['depreciate_notice_list'], 'href' => 'sale_notice.php?act=list'));
	assign_query_info();
...
if ($_REQUEST['act'] == 'action') {
	...
	if (isset($_POST['resms']) && !empty($detail['cellphone']) && $_CFG['sms_price_notice'] == 1) {
		...
		$smsParams = array('user_name' => $detail['user_name'], 'username' => $detail['user_name'], 'goodsname' => sub_str($detail['goods_name'], 20), 'goodsprice' => $detail['shop_price'], 'mobile_phone' => $detail['cellphone'] ? $detail['cellphone'] : '', 'mobilephone' => $detail['cellphone'] ? $detail['cellphone'] : '');
