	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
				...
				else if ($_REQUEST['act'] == 'edit_sales_volume_base') {
					check_authz_json('goods_manage');
					$goods_id = intval($_POST['id']);
					$sales_volume_base = $_POST['val'];
							
					if ($sales_volume_base == '') {
						make_json_error($_LANG['sales_volume_invalid']);
					}
					else if ($exc->edit('sales_volume_base = \'' . $sales_volume_base . '\', last_update=' . gmtime(), $goods_id)) {
						clear_cache_files();
						make_json_result($sales_volume_base);
					}
				}
					...
					if ($data['status'] != 0) {
						...
						else if (strpos($data['result']['grossweight'], $_LANG['unit_g']) !== false) {
