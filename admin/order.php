...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		case 1:
			$where .= ' AND o.pay_status = \'' . PS_UNPAYED . '\' AND o.order_status = \'' . OS_CONFIRMED . '\' ';
			break;

		case 2:
			$where .= ' AND o.shipping_status = \'' . SS_SHIPPED . '\' ';
			break;
		...
		case 4:
			$where .= ' AND o.pay_status = \'' . PS_PAYING . '\' ';
			break;
		...
		case 6:
			$where .= ' AND o.order_status = \'' . OS_INVALID . '\' ';
			break;

		case 7:
			$leftJoin .= ' JOIN ' . $GLOBALS['ecs']->table('order_return') . ' AS ore ON ore.order_id = o.order_id ';
			$where .= 'AND o.order_status NOT IN (\'' . OS_CANCELED . '\', \'' . OS_INVALID . '\', \'' . OS_RETURNED . '\', \'' . OS_RETURNED_PART . '\', \'' . OS_ONLY_REFOUND . '\') ';
			break;

		case 8:
			$where .= ' AND (o.shipping_status = \'' . SS_UNSHIPPED . '\' OR o.shipping_status = \'' . SS_PREPARING . '\') AND o.pay_status = \'' . PS_PAYED . '\' AND o.order_status = \'' . OS_CONFIRMED . '\' AND (SELECT ore.ret_id FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS ore WHERE ore.order_id = o.order_id LIMIT 1) IS NULL ';
		}

		if (!empty($filter['keywords'])) {
			$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS iog ON iog.order_id = o.order_id ';
			$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $status_where . $where_store . $no_main_order . ' GROUP BY o.order_id';
...
function delivery_list()
{
	...
	foreach ($row as $key => $value) {
		...
		else if ($value['status'] == DELIVERY_CREATE) {
...
if ($_REQUEST['act'] == 'order_download') {
	...
	while ($row = $db->fetchRow($res)) {
		...
		$row['package_goods_list'] = get_package_goods_list($row['goods_id']);
		$sql = 'SELECT ret_id, agree_apply, return_status, refound_status FROM ' . $ecs->table('order_return') . (' WHERE rec_id = \'' . $row['rec_id'] . '\' AND return_type = 3 ');
	...
	$goods_sql = 'SELECT dg.*, g.goods_thumb, g.brand_id  FROM ' . $ecs->table('delivery_goods') . ' AS dg LEFT JOIN ' . $ecs->table('goods') . ' as g ON dg.goods_id = g.goods_id ' . ' WHERE delivery_id = ' . $delivery_order['delivery_id'];
	$goods_list = $GLOBALS['db']->getAll($goods_sql);
	...
	$_delivery['status'] = DELIVERY_SHIPPED;
	...
	if ($order_finish) {
		...
		if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
			...
			$smsParams = array('shop_name' => $shop_name, 'shopname' => $shop_name, 'user_name' => $user_info['user_name'], 'username' => $user_info['user_name'], 'consignee' => $order['consignee'], 'order_sn' => $order['order_sn'], 'ordersn' => $order['order_sn'], 'invoice_no' => $order['invoice_no'], 'invoiceno' => $order['invoice_no'], 'mobile_phone' => $order['mobile'], 'mobilephone' => $order['mobile']);
		...
		if (file_exists($file)) {
			$pushData = array(
				'first'    => array('value' => $_LANG['order_first']),
				...
				'remark'   => array('value' => $_LANG['order_remark'])
				);
	...
	$order['insure_yn'] = empty($order['insure_fee']) ? 0 : 1;
	$goods_sql = 'SELECT b.*, g.goods_thumb, g.brand_id FROM ' . $ecs->table('back_goods') . ' AS b LEFT JOIN ' . $ecs->table('goods') . ' AS g ON g.goods_id = b.goods_id ' . ' WHERE back_id = ' . $back_order['back_id'];
	$goods_list = $GLOBALS['db']->getAll($goods_sql);
	...
	if ('user' == $step) {
		...
		$order = array('user_id' => $user_id, 'add_time' => gmtime(), 'order_status' => OS_INVALID, 'shipping_status' => SS_UNSHIPPED, 'pay_status' => PS_UNPAYED, 'from_ad' => 0, 'referer' => $_LANG['admin'], 'is_delete' => 2);
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
		...
		if (isset($_POST['confirm'])) {
			...
			if (!empty($invoice_no) && !$delivery_info) {
				...
				$delivery['status'] = DELIVERY_CREATE;
						...
						foreach ($goods_list as $value) {
							if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card' || $value['extension_code'] == 'presale' || substr($value['extension_code'], 0, 7) == 'seckill') {
								...
								foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
									$delivery_pg_goods = array('delivery_id' => $delivery_id, 'goods_id' => $pg_value['goods_id'], 'product_id' => $pg_value['product_id'], 'product_sn' => $pg_value['product_sn'], 'goods_name' => $pg_value['goods_name'], 'brand_name' => '', 'goods_sn' => $pg_value['goods_sn'], 'send_number' => $pg_value['order_goods_number'], 'parent_id' => $value['goods_id'], 'extension_code' => $value['extension_code'], 'is_real' => $pg_value['is_real']);
				...
				if ($order_finish) {
					...
					if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
						...
						$user_info = get_admin_user_info($order['user_id']);
						$smsParams = array('shop_name' => $shop_name, 'shopname' => $shop_name, 'user_name' => $user_info['user_name'], 'username' => $user_info['user_name'], 'consignee' => $order['consignee'], 'order_sn' => $order['order_sn'], 'ordersn' => $order['order_sn'], 'invoice_no' => $invoice_no, 'invoiceno' => $invoice_no, 'mobile_phone' => $order['mobile'], 'mobilephone' => $order['mobile']);
				...
				clear_cache_files();
				$seller_list = 0 < $order['ru_id'] ? '&seller_list=1' : '&seller_list=0';
			...
			if ($goods_list) {
				foreach ($goods_list as $key => $goods_value) {
					...
					if ($goods_value['extension_code'] == 'package_buy' && 0 < count($goods_value['package_goods_list'])) {
						...
						$sql = 'SELECT ret_id, agree_apply, return_status, refound_status FROM ' . $ecs->table('order_return') . (' WHERE rec_id = \'' . $goods_value['rec_id'] . '\' AND return_type = 3 ');
						$rec_info = $db->getRow($sql);
	...
	array('href' => 'order.php?act=back_list', 'text' => $_LANG['return_list'])
			...
			$seller_list = 0 < $user_id ? '&seller_list=1' : '&seller_list=0';
			...
			sys_msg($_LANG['batch_delivery_success'], 0, array(
	array('href' => 'order.php?act=delivery_list', 'text' => $_LANG['return_list'])
	));
		}
		...
		if ('confirm' == $operation) {
			foreach ($order_id_list as $id_order) {
				$sql = 'SELECT * FROM ' . $ecs->table('order_info') . (' WHERE order_sn = \'' . $id_order . '\'') . ' AND order_status = \'' . OS_UNCONFIRMED . '\' LIMIT 1';
				$order = $db->getRow($sql);
			...
			foreach ($order_id_list as $id_order) {
				$sql = 'SELECT * FROM ' . $ecs->table('order_info') . (' WHERE order_sn = ' . $id_order) . order_query_sql('unpay_unship') . ' LIMIT 1';
				$order = $db->getRow($sql);
	...
	array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])
		...
		$goods_delivery = isset($_REQUEST['delivery']) && !empty($_REQUEST['delivery']) ? $_REQUEST['delivery'] : array();
		$return = '';
		...
		if ('confirm' == $operation) {
			...
			update_order($order_id, $arr);
			if (0 < $order['money_paid'] && $order['money_paid'] != PS_UNPAYED) {
			...
			$arr = array('order_status' => OS_RETURNED, 'pay_status' => PS_UNPAYED, 'shipping_status' => SS_UNSHIPPED, 'money_paid' => 0, 'invoice_no' => '', 'return_amount' => $order_return_amount, 'order_amount' => $order_return_amount);
			update_order($order_id, $arr);
