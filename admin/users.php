...
function user_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT u.user_rank,u.user_id, u.user_name, u.nick_name, u.mobile_phone, u.email, u.is_validated, u.user_money, u.frozen_money, u.rank_points, u.pay_points, u.reg_time,rank_points ' . ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' . $ex_where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . $limit;
...
if ($_REQUEST['act'] == 'list') {
	...
	$sql = 'SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , ' . 'u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone, ' . 'u.question, u.answer, r.front_of_id_card,r.real_id, r.reverse_of_id_card ' . ' FROM ' . $ecs->table('users') . ' u LEFT JOIN ' . $ecs->table('users') . ' u2 ON u.parent_id = u2.user_id ' . ' LEFT JOIN ' . $ecs->table('users_real') . ' r ON u.user_id = r.user_id ' . (' WHERE u.user_id = \'' . $user_id . '\'');
	$row = $db->GetRow($sql);
	$row['user_name'] = addslashes($row['user_name']);
	$users = &init_users();
	$user = $users->get_user_info($row['user_name']);
	$sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,\r\n            u.office_phone, u.home_phone, u.mobile_phone," . 'u.passwd_question, u.passwd_answer, r.front_of_id_card,r.real_id, r.reverse_of_id_card' . ' FROM ' . $ecs->table('users') . ' u LEFT JOIN ' . $ecs->table('users') . ' u2 ON u.parent_id = u2.user_id ' . ' LEFT JOIN ' . $ecs->table('users_real') . ' r ON u.user_id = r.user_id ' . (' WHERE u.user_id = \'' . $user_id . '\'');
	$row = $db->GetRow($sql);
