...
if ($_REQUEST['act'] == 'get_select_category') {
	...
	foreach ($arr as $val) {
		$opt[] = array('value' => $val['id'], 'text' => $_LANG['sec_price_alt'] . $val['sec_price'] . $_LANG['sec_number_alt'] . $val['sec_num'] . $_LANG['sec_restrictions_alt'] . $val['sec_limit'] . '】' . $val['goods_name'], 'data' => '');
	}
	...
	if (!empty($result['sort'])) {
		foreach ($result['sort'] as $k => $v) {
			...
			$sql = 'SELECT a.article_id, a.content, a.title, a.add_time, a.description, a.file_url, a.open_type ' . ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a ' . ' WHERE a.is_open = 1 ' . $where . ' ORDER BY a.article_type DESC, a.sort_order ASC, a.article_id DESC LIMIT 5';
	...
	$result['article_title'] = !empty($_REQUEST['article_title']) ? trim($_REQUEST['article_title']) : $_LANG['recent_hot'];
	$result['goods_title'] = !empty($_REQUEST['goods_title']) ? trim($_REQUEST['goods_title']) : $_LANG['best_recommend'];
