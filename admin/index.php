...
function get_menu_list($menus, $ru_id = 0)
{
	...
	foreach ($menus as $key => $row) {
		...
		else if ($row['label'] == $GLOBALS['_LANG']['03_goods_storage']) {
			$arr[$key]['href'] = 'goods_inventory_logs.php?act=list&step=put';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['05_banner']) {
			if (0 < $ru_id) {
		...
		else if ($row['label'] == $GLOBALS['_LANG']['04_order']) {
			$number = get_order_count($ru_id);
			$arr[$key]['number'] = $number;
			$arr[$key]['href'] = 'order.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['20_suppliers_promotion']) {
			$arr[$key]['href'] = 'bonus.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['report_form_stats']) {
			$arr[$key]['href'] = 'order_stats.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['07_content']) {
			$arr[$key]['href'] = 'articlecat.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['user_manage']) {
			$arr[$key]['href'] = 'users.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['10_priv_admin']) {
			$arr[$key]['href'] = 'privilege.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['11_system']) {
			if (0 < $ru_id) {
		...
		else if ($row['label'] == $GLOBALS['_LANG']['03_template']) {
			$arr[$key]['href'] = 'template.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['13_backup']) {
			$arr[$key]['href'] = 'sql.php?act=main';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['14_sms']) {
			$arr[$key]['href'] = 'sms.php?act=display_send_ui';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['15_rec']) {
			$arr[$key]['href'] = 'affiliate.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['16_email_manage']) {
			$arr[$key]['href'] = 'view_sendlist.php?act=list';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['seller_apply_manage']) {
			if (0 < $ru_id) {
		...
		else if ($row['label'] == $GLOBALS['_LANG']['goods_batch_manage']) {
			$arr[$key]['href'] = 'goods_warehouse_batch.php?act=add';
		}
		else if ($row['label'] == $GLOBALS['_LANG']['store_set_manage']) {
...
if ($_REQUEST['act'] == '') {
	...
	$series = array(
		array(
			...
			'markPoint' => array(
				...
				'data'      => array(
					array('type' => 'max', 'name' => $_LANG['max']),
					array('type' => 'min', 'name' => $_LANG['min'])
					)
				)
			),
		array(
	...
	$orders_yAxis = array(
		'type'      => 'value',
		'axisLabel' => array('formatter' => '{value}' . $_LANG['ge'])
		);
	$orders_series = array(
		array(
	...
	$sales_yAxis = array(
		'type'      => 'value',
		'axisLabel' => array('formatter' => '{value}' . $_LANG['yuan'])
		);
	$sales_series = array(
		array(
