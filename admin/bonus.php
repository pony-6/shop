...
function get_type_list($ru_id)
{
	...
	if ($result === false) {
		...
		$where .= !empty($filter['seller_list']) ? ' AND bt.user_id > 0 ' : ' AND bt.user_id = 0 ';
...
if ($_REQUEST['act'] == 'remove') {
	...
	if (isset($_POST['type'])) {
		if ($_POST['type'] == 'batch_remove') {
			$sql = 'DELETE FROM ' . $ecs->table('bonus_type') . ' WHERE type_id ' . db_create_in($ids);
			...
			$sql = 'UPDATE ' . $ecs->table('bonus_type') . (' SET review_status = \'' . $review_status . '\' ') . ' WHERE type_id ' . db_create_in($ids);
