...
if ($_REQUEST['act'] == 'add_shop' || $_REQUEST['act'] == 'edit_shop' || $_REQUEST['act'] == 'copy_shop') {
	...
	if ($_REQUEST['act'] == 'insert_shop' || $_REQUEST['act'] == 'update_shop') {
	...
	else if ($_REQUEST['act'] == 'see_shopinfo') {
		admin_priv('users_merchants');
		$user_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
		$adminru['ru_id'] = $user_id;
		...
		$smarty->assign('action_link', array('text' => $_LANG['back'], 'href' => 'merchants_users_list.php?act=list'));
		$smarty->assign('ru_id', $adminru['ru_id']);
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as mis ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' as u on mis.user_id = u.user_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' as ss on mis.user_id = ss.ru_id ' . ' WHERE ss.ru_id = \'' . $user_id . '\' LIMIT 1';
		$shop_information = $db->getRow($sql);
		$shop_name = get_shop_name($adminru['ru_id']);
		$shop_information['is_dsc'] = $adminru['ru_id'] == 0 ? true : false;
		...
		$merch_cmt = get_merchants_goods_comment($user_id);
	...
	else if ($_REQUEST['act'] == 'ajax_send_message') {
		check_authz_json('users_merchants');
		$user_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
		...
		$send_date = local_date($GLOBALS['_CFG']['time_format'], gmtime());
		if ($type == 1 && !empty($to_email)) {
			$template = array('template_subject' => '店铺等级到期时间提醒', 'is_html' => 1, 'template_content' => "亲爱的{\$user_name}，你好！\r\n\r\n            您的店铺等级到期时间快过期了，到期时间：\r\n            “{\$grade_end_time}”\r\n\r\n            如需继续使用，请您及时续费，以免造成不必要的损失。\r\n            {\$shop_name}\r\n            {\$send_date}");
			$smarty->assign('user_name', $user_name);
		...
		if ($type == 0 && !empty($to_mobile)) {
			$pt_smsParams = array('user_name' => $user_name, 'username' => $user_name, 'grade_end_time' => $grade_end_time, 'gradeendtime' => $grade_end_time, 'shop_name' => $shop_name, 'shopname' => $shop_name, 'send_date' => $send_date, 'senddate' => $send_date, 'mobile_phone' => $to_mobile, 'mobilephone' => $to_mobile);

			if ($GLOBALS['_CFG']['sms_type'] == 0) {
				$resp = huyi_sms($pt_smsParams, 'sms_seller_grade_time');
			}
			else if (1 <= $GLOBALS['_CFG']['sms_type']) {
				$pt_result = sms_ali($pt_smsParams, 'sms_seller_grade_time');
