...
if ($_REQUEST['act'] == 'list') {
	...
	if ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping'])) {
		...
		$shipping_id = isset($_POST['shipping']) && !empty($_POST['shipping']) ? intval($_POST['shipping']) : 0;
		$sql = 'SELECT COUNT(*) FROM ' . $ecs->table('shipping_area') . (' WHERE shipping_id=\'' . $_POST['shipping'] . '\' AND ') . ('shipping_area_name=\'' . $shipping_area_name . '\' AND ') . ('shipping_area_id <> \'' . $shipping_area_id . '\' and ru_id=\'') . $adminru['ru_id'] . '\'';

		if (0 < $db->getOne($sql, true)) {
			sys_msg($_LANG['repeat_area_name'], 1);
		}
		else {
			$shipping_code = $db->getOne('SELECT shipping_code FROM ' . $ecs->table('shipping') . (' WHERE shipping_id=\'' . $_POST['shipping'] . '\''));
			...
			$sql = 'UPDATE ' . $ecs->table('shipping_area') . (' SET shipping_area_name=\'' . $shipping_area_name . '\', ') . 'configure=\'' . serialize($config) . '\' ' . ('WHERE shipping_area_id = \'' . $shipping_area_id . '\' and ru_id=\'') . $adminru['ru_id'] . '\'';
			$db->query($sql);
			...
			if ($point_name && $mobile && $address) {
				foreach ($point_name as $key => $val) {
					...
					if ($point_id[$key]) {
						...
						$sql = 'INSERT INTO ' . $ecs->table('shipping_point') . ' (shipping_area_id, name, user_name, mobile, address, img_url, anchor, line) ' . (' VALUES (\'' . $shipping_area_id . '\', \'' . $point_name[$key] . '\' , \'' . $user_name[$key] . '\' ,\'' . $mobile[$key] . '\' , \'' . $address[$key] . '\', \'' . $map_img . '\', \'' . $anchor[$key] . '\', \'' . $line[$key] . '\')');
					}
