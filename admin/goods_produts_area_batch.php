...
if ($_REQUEST['act'] == 'add') {
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		if (!empty($result['list']['goods_attr'])) {
			...
			$sql = 'SELECT product_id FROM ' . $GLOBALS['ecs']->table('products_area') . ' WHERE goods_id = \'' . $result['list']['goods_id'] . '\'' . ' AND area_id = \'' . $result['list']['area_id'] . '\'' . ' AND city_id = \'' . $result['list']['city_id'] . '\'' . $where;
