...
function get_user_default($user_id = 0)
{
	...
	if (0 < $row['user_rank']) {
		$sql = 'SELECT rank_id, rank_name, discount, special_rank FROM ' . $GLOBALS['ecs']->table('user_rank') . (' WHERE rank_id = \'' . $row['user_rank'] . '\'');
	}
	else {
		$sql = 'SELECT rank_id, rank_name, discount, min_points, special_rank' . ' FROM ' . $GLOBALS['ecs']->table('user_rank') . ' WHERE min_points <= ' . intval($row['rank_points']) . ' ORDER BY min_points DESC';
	}
...
function get_default_collection_goods($user_id)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$goods_list[$row['goods_id']]['is_IM'] = $shop_information['is_IM'];
...
function get_shop_name($ru_id = 0, $type = 0)
{
	...
	if ($type == 1) {
		...
		$res = array('shop_name' => $shop_name, 'shopNameSuffix' => $shop_information['shopNameSuffix'], 'shopinfo' => $shopinfo, 'shop_information' => $shop_information);
		return $res;
	}
	else {
...
function get_discuss_all_list($goods_id = 0, $dis_type = 0, $reply_page = 1, $size = 40, $revType = 0, $sort = 'add_time', $did = 0)
{
	...
	if ($dis_type == 4) {
		...
		$sql = 'SELECT DC1.dis_id, DC1.dis_type, DC1.dis_title, DC1.user_name, DC1.add_time, DC1.dis_browse_num, u.nick_name, ' . '(SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('discuss_circle') . ' AS DC2' . ' WHERE DC2.parent_id = DC1.dis_id AND DC2.review_status = 3) AS reply_num' . ' FROM ' . $GLOBALS['ecs']->table('discuss_circle') . ' AS DC1' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id = DC1.user_id ' . (' WHERE ' . $where . ' AND DC1.parent_id = 0 AND DC1.goods_id = \'' . $goods_id . '\' AND DC1.dis_id <> \'' . $did . '\' AND DC1.review_status = 3 ORDER BY ' . $sort . ' DESC ') . $limit;
		$res = $GLOBALS['db']->getAll($sql);
		$arr = array();
...
function presale_info($presale_id, $current_num = 0, $user_id = 0, $path = '')
{
	...
	$sql = 'SELECT b.*, b.cat_id AS pa_catid,g.goods_name, g.shop_price, g.goods_desc, g.user_id, g.goods_id, g.goods_product_tag, g.cat_id, b.review_status, b.review_content, ' . 'g.xiangou_start_date, g.xiangou_end_date, g.xiangou_num, g.is_xiangou, b.user_id AS ru_id, g.brand_id ' . 'FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON b.goods_id = g.goods_id ' . ('WHERE b.act_id = \'' . $presale_id . '\' ' . $where) . ' LIMIT 1';
...
function get_order_goods_buy_list($warehouse_id = 0, $area_id = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		if($row['sales_volume_base'] > 0){
	    $arr[$key]['sales_volume'] = $row['sales_volume']+$row['sales_volume_base'];
        }else{$arr[$key]['sales_volume'] = $row['sales_volume'];}
...
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
	...
	if ($query) {
		while ($query && ($row = $GLOBALS['db']->fetch_array($query))) {
			...
			if($row['sales_volume_base'] > 0){
			$guess_goods[$row['goods_id']]['sales_volume'] = $row['sales_volume']+$row['sales_volume_base'];
			}else{$guess_goods[$row['goods_id']]['sales_volume'] = $row['sales_volume'];}
...
function get_category_store_list($ru_id = 0, $is_url = 0, $level = 0, $parent_id = 0)
{
	...
	$sql = 'SELECT cat_id, cat_name, parent_id, user_id AS ru_id, sort_order, ' . 'measure_unit, grade, is_show, show_in_nav FROM ' . $GLOBALS['ecs']->table('merchants_category') . (' WHERE ' . $where . ' ORDER BY cat_id , sort_order ') . ' LIMIT ' . $filter['start'] . (',' . $filter['page_size']);
	$res = $GLOBALS['db']->getAll($sql);
...
function sms_ali($params = array(), $send_time = '', $test = array())
{
	...
	if ($sms_type == 2) {
		...
		$result = array('TemplateContent' => !empty($test['temp_content']) ? $test['temp_content'] : $row['temp_content'], 'TemplateParam' => json_encode($smsParams), 'PhoneNumbers' => $params['mobile_phone']);
	}
	else {
...
function get_goods_lib_cat($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			$sql = 'SELECT glc.cat_id, glc.cat_name, glc.parent_id, glc.sort_order, COUNT(s.parent_id) AS has_children ' . ' FROM ' . $GLOBALS['ecs']->table('goods_lib_cat') . ' AS glc ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_lib_cat') . ' AS s ON s.parent_id = glc.cat_id ' . ' GROUP BY glc.cat_id ' . ' ORDER BY glc.parent_id, glc.sort_order ASC';
			$res = $GLOBALS['db']->getAll($sql);
	...
	$options = goods_lib_cat_options($cat_id, $res);
	$children_level = 99999;
...
function shipping_info($shipping, $select = array())
{
	...
	if (is_array($shipping)) {
		...
		else if (isset($shipping['shipping_id'])) {
	...
	$sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('shipping') . (' WHERE 1 ' . $where . ' AND enabled = 1 LIMIT 1');
	$row = $GLOBALS['db']->getRow($sql);
...
function get_baitiao_log_info($other = array())
{
	...
	if ($other) {
		...
		if ($row['is_stages'] == 1) {
			...
			$row['repay_date'] = $repay_date[$row['yes_num'] + 1];
		}
		else {
...
{
	$sql = 'SELECT b.*, b.stages_one_price * b.stages_total AS order_amount, o.order_sn, o.pay_id FROM ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . '  AS o ON b.order_id = o.order_id ' . (' WHERE b.user_id = \'' . $user_id . '\' ORDER BY b.log_id DESC ');
		...
		while ($row = $GLOBALS['db']->fetchRow($res)) {
			if ($row['stages_total'] && $row['is_repay'] == 0) {
				for ($i = 1; $i <= $row['stages_total']; $i++) {
					$pay_log_other = array('baitiao_id' => $row['baitiao_id'], 'log_id' => $row['log_id'], 'stages_num' => $i, 'is_pay' => 0);
					$log_info = get_baitiao_pay_log_info($pay_log_other);
					if ($log_info && $log_info['pay_id']) {
						$payment = array('pay_id' => $log_info['pay_id'], 'pay_code' => $log_info['pay_code']);
						...
						if ($payment && file_exists($file_pay)) {
							...
							if (class_exists($payment['pay_code'])) {
								...
								if (is_callable($is_callable)) {
									$order_other = array('order_sn' => $row['order_sn'], 'log_id' => $log_info['id']);
			...
			if ($row['is_stages'] == 1) {
				...
				$row['repay_date'] = $repay_date[$stages_num];
			}
			else {
				$row['repay_date'] = local_date($GLOBALS['_CFG']['date_format'], $row['repay_date']);
			}
			...
			if ($row['pay_num'] == 0 && 0 < $row['stages_total'] && $row['is_stages'] == 1) {
				for ($i = 1; $i <= $row['stages_total']; $i++) {
					...
					if (!$GLOBALS['db']->getOne($sql, true)) {
						$pay_log_other = array('log_id' => $row['log_id'], 'baitiao_id' => $row['baitiao_id'], 'stages_num' => $i, 'stages_price' => $row['stages_one_price'], 'add_time' => gmtime());
...
function get_baitiao_balance($user_id = 0)
{
	...
	if ($bt_info) {
		...
		$arr = array('amount' => $bt_info['amount'], 'balance' => $remain_amount, 'numbers' => $baitiao_log['numbers'], 'stay_pay' => $baitiao_log['total_amount'], 'already_amount' => $baitiao_log['already_amount'], 'bt_info' => $bt_info);
	}
...
function get_baitiao_pay_log_list($log_id = 0, $size = 0, $start = 0)
{
	...
	if ($res) {
		while ($row = $GLOBALS['db']->fetchRow($res)) {
			...
			$sql = 'SELECT o.order_id, o.order_sn, o.pay_id FROM ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . '  AS o ON b.order_id = o.order_id ' . ' WHERE b.log_id = \'' . $row['log_id'] . '\' ';
