...
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0, $select = array())
{
	...
	if ($select) {
		...
		$select = ' g.*, IF(g.model_inventory < 1, g.goods_number, IF(g.model_inventory < 2, wg.region_number, wag.region_number)) as goods_number,' . (' IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\'), g.shop_price * \'' . $_SESSION['discount'] . '\')  AS shop_price,') . ' IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price,' . ' IF(g.model_price < 1, g.integral, IF(g.model_price < 2, wg.pay_integral, wag.pay_integral)) as integral,' . ' c.measure_unit, g.brand_id as brand_id, g.comments_number, g.sales_volume, g.sales_volume_base, ' . ('IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS rank_price, ') . 'IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS price ';
	}
...
function get_goods_fittings_info($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $rev = '', $type = 0, $fittings_goods = 0, $fittings_attr = array())
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$attr_number = !empty($attr_number) ? $attr_number : 0;
		$arr[$temp_index]['goods_number'] = $attr_number;
		$arr[$temp_index]['properties'] = get_goods_properties($goods_id, $warehouse_id, $area_id, $area_city, 0, $row['goods_attr_id']);
		$temp_index++;
	}
