...
function addto_cart_combo($goods_id, $num = 1, $spec = array(), $parent = 0, $group = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_attr = '')
{
	...
	$goods_attr = get_goods_attr_info($spec, 'pice', $warehouse_id, $area_id);
	$goods_attr_id = join(',', $spec);
	$parent = array('user_id' => $_SESSION['user_id'], 'session_id' => $sess, 'goods_id' => $goods_id, 'goods_sn' => addslashes($goods['goods_sn']), 'product_id' => $product_info['product_id'], 'goods_name' => addslashes($goods['goods_name']), 'market_price' => $goods['market_price'], 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $goods_attr_id, 'is_real' => $goods['is_real'], 'model_attr' => $goods['model_attr'], 'warehouse_id' => $warehouse_id, 'area_id' => $area_id, 'area_city' => $area_city, 'ru_id' => $goods['ru_id'], 'extension_code' => $goods['extension_code'], 'is_gift' => 0, 'model_attr' => $goods['model_attr'], 'commission_rate' => $goods['commission_rate'], 'is_shipping' => $goods['is_shipping'], 'rec_type' => CART_GENERAL_GOODS, 'add_time' => gmtime(), 'group_id' => $group);
	$basic_list = array();
...
function compute_discount($type = 0, $newInfo = array(), $use_type = 0, $ru_id = 0)
{
	...
	if ($type == 0 || $type == 3) {
		...
		$sql = 'SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, c.ru_id, c.act_id ' . 'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE c.goods_id = g.goods_id ' . 'AND ' . $c_sess . 'AND c.parent_id = 0 ' . 'AND c.is_gift = 0 ' . 'AND rec_type = \'' . CART_GENERAL_GOODS . '\'' . $where;
		$goods_list = $GLOBALS['db']->getAll($sql);
	}
	else if ($type == 2) {
		$goods_list = array();
...
function cart_by_favourable($merchant_goods)
{
	$id_list = array();
	$list_array = array();
	$cart_value['act_sel_id'] = empty($_SESSION['cart_value']) ? array() : explode(',', $_SESSION['cart_value']);
							...
							if ($fav_row['userFav_type'] == 1 || $mer_ids) {
								if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0) {
									if (isset($goods_row) && $goods_row) {
										...
										if ($merchant_goods[$key]['new_list'][$fav_row['act_id']]['available'] && $fav_row['act_type'] == 2) {
					...
					if (in_array($cat_id, $id_list) && $goods_row['is_gift'] == 0 || in_array($goods_row['is_gift'], $favourable_id_list)) {
						foreach ($sort_favourable['by_category'] as $fav_key => $fav_row) {
							if (isset($goods_row) && $goods_row && ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id']))) {
								...
								if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 && in_array($cat_id, $fav_act_range_ext)) {
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_id'] = $fav_row['act_id'];
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_name'] = $fav_row['act_name'];
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type'] = $fav_row['act_type'];
				...
				if (isset($sort_favourable['by_brand']) && $goods_row['extension_code'] != 'package_buy' && substr($goods_row['extension_code'], 0, 7) != 'seckill') {
					...
					if (in_array(trim($brand_id), $get_act_range_ext) && $goods_row['is_gift'] == 0 || in_array($goods_row['is_gift'], $favourable_id_list)) {
						foreach ($sort_favourable['by_brand'] as $fav_key => $fav_row) {
							...
							if (isset($goods_row) && $goods_row && ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id']))) {
								if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 && strstr($act_range_ext_str, trim($brand_id_str))) {
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_id'] = $fav_row['act_id'];
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_name'] = $fav_row['act_name'];
									$merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type'] = $fav_row['act_type'];
	...
	foreach ($merchant_goods as $key => $row) {
		...
		if ($row['ru_id'] == $favourable_activity['user_id']) {
			foreach ($user_cart_goods as $goods_key => $goods_row) {
				...
				if (!empty($act_sel_id)) {
					$goods_row['sel_checked'] = strstr(',' . $act_sel_id['act_sel_id'] . ',', ',' . $goods_row['rec_id'] . ',') ? 1 : 0;
				}

				if ($goods_row['act_id'] == $favourable_activity['act_id']) {
					if ($favourable_activity['act_range'] == 0 && $goods_row['extension_code'] != 'package_buy') {
						if ($goods_row['is_gift'] == FAR_ALL && $goods_row['parent_id'] == 0) {
							...
							if ($favourable_box['available'] && $favourable_activity['act_type'] == 2) {
...
function get_favourable_info($goods_id = 0, $ru_id = 0, $goods = array())
{
	$gmtime = gmtime();
