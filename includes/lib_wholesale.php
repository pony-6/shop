...
function get_purchase_list($filter = array(), $size = 10, $page = 1, $sort = 'add_time', $order = 'DESC')
{
	...
	if (isset($filter['is_finished'])) {
		...
		else if ($filter['is_finished'] == 1) {
			$where .= ' AND end_time < \'' . $now . '\' ';
		}
		else if ($filter['is_finished'] == 2) {
		}
	}
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$row['left_day'] = 0 < $row['left_day'] ? intval($row['left_day']) : 0;
...
function get_purchase_info($purchase_id = 0)
{
	...
	if ($purchase_info) {
		...
		foreach ($goods_list as $key => $val) {
			$goods_img = $val['goods_img'] ? unserialize($val['goods_img']) : array();
		...
		$purchase_info['left_day'] = 0 < $purchase_info['left_day'] ? $purchase_info['left_day'] : 0;
...
function get_seller_area_info($ru_id = 0, $type = 0)
{
	...
	default:
		$data = array('country', 'province', 'city');
		break;
	}
...
function attrs_group()
{
	...
	if (func_num_args() == 1) {
		return call_user_func_array('attrs_group', $t[0]);
	}
...
function handle_wholesale_volume_price($goods_id, $is_volume, $number_list, $price_list, $id_list)
{
	if ($is_volume) {
		foreach ($price_list as $key => $price) {
			...
			if (!empty($price)) {
				if ($volume_id) {
					...
					if (!$GLOBALS['db']->getOne($sql)) {
						$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('wholesale_volume_price') . ' (price_type, goods_id, volume_number, volume_price) ' . ('VALUES (\'1\', \'' . $goods_id . '\', \'' . $volume_number . '\', \'' . $price . '\')');
						$GLOBALS['db']->query($sql);
					}
				}
			}
		...
		$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('wholesale_volume_price') . (' WHERE price_type = \'1\' AND goods_id = \'' . $goods_id . '\'');
		$GLOBALS['db']->query($sql);
	}
}
...
function get_wholesale_volume_price_list($goods_id, $price_type = '1')
{
	$volume_price = array();
	$temp_index = '0';
	$sql = 'SELECT `id` , `volume_number` , `volume_price`' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_volume_price') . '' . ' WHERE `goods_id` = \'' . $goods_id . '\' AND `price_type` = \'' . $price_type . '\'' . ' ORDER BY `volume_number`';
...
{
	$sql = ' SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values ' . ' FROM ' . $GLOBALS['ecs']->table('attribute') . ' AS a ' . ' WHERE a.cat_id = ' . intval($goods_type) . ' AND a.cat_id <> 0 ' . ' AND a.attr_type <> 2 ' . ' ORDER BY a.sort_order, a.attr_type, a.attr_id ';
	$attribute_list = $GLOBALS['db']->getAll($sql);
	$sql = ' SELECT v.goods_attr_id, v.attr_id, v.attr_value, v.attr_price, v.attr_sort, v.attr_checked, v.attr_img_flie, v.attr_gallery_flie  ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' AS v ' . (' WHERE v.goods_id = \'' . $goods_id . '\' ORDER BY v.attr_sort, v.goods_attr_id ');
...
function get_wholesale_goods_attr_id($where_select = array(), $select = array(), $attr_type = 0, $retuen_db = 0)
{
	...
	$sql = ' SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' AS ga, ' . $GLOBALS['ecs']->table('attribute') . ' AS a' . (' WHERE ga.attr_id = a.attr_id ' . $where);
...
function get_wholesale_product_attr_list($goods_attr_id = 0, $goods_id = 0, $goods_model = 0, $warehouse_id = 0, $area_id = 0)
{
	...
	$sql = 'SELECT  ga.goods_attr_id, ga.attr_id, ga.attr_value ' . $select . ' FROM  ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' AS ga ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = ga.attr_id ' . $leftJion . (' WHERE ga.goods_attr_id IN(' . $goods_attr_id . ') AND ga.goods_id = \'' . $goods_id . '\'') . ' ORDER BY a.sort_order, a.attr_id, ga.goods_attr_id';
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
...
function handle_wholesale_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
	...
	foreach ($id_list as $key => $id) {
		...
		if (!empty($result_id)) {
			...
			$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' (goods_id, attr_id, attr_value, attr_price) ' . ('VALUES (\'' . $goods_id . '\', \'' . $id . '\', \'' . $value . '\', \'' . $price . '\')');
		}
...
function get_wholesale_goods_info($act_id, $warehouse_id = 0, $area_id = 0, $select = array())
{
	...
	$sql = ' SELECT w.goods_id, w.goods_name, w.goods_type, w.rank_ids, w.price_model, ' . $goods_price . ', w.goods_number, w.moq, ' . ' wc.cat_id, ' . ' g.user_id, g.market_price, g.shop_price, g.goods_desc, g.goods_img, g.brand_id, g.goods_sn, g.goods_weight, g.goods_unit FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . $left_join . $where;
	...
	if ($row !== false) {
		...
		$build_uri = array('urid' => $row['user_id'], 'append' => $arr['rz_shopName']);
...
function get_wholesale_goods_attr_ajax($goods_id, $goods_attr, $goods_attr_id)
{
	...
	if ($goods_attr) {
		...
		$sql = 'SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value  FROM ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' AS ga' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON ga.attr_id = a.attr_id ' . (' WHERE  ga.goods_id = \'' . $goods_id . '\' ' . $where . ' AND a.attr_type > 0 ORDER BY a.sort_order, a.attr_id, ga.goods_attr_id');
		$res = $GLOBALS['db']->getAll($sql);
...
function see_more_goods($user_id, $act_id)
{
	...
	$sql = ' SELECT w.act_id, ' . $goods_price . ', g.goods_name, g.goods_thumb, g.user_id FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = w.goods_id ' . (' WHERE g.user_id = \'' . $user_id . '\' AND w.act_id <> \'' . $act_id . '\' AND enabled = 1 ORDER BY act_id DESC LIMIT 5 ');
	$res = $GLOBALS['db']->getAll($sql);
...
function wholesale_cart_goods($goods_id = 0, $rec_ids = '')
{
	...
	foreach ($ru_ids as $key => $val) {
		...
		foreach ($goods_ids as $a => $g) {
			...
			$sql = ' SELECT c.rec_id, c.goods_price, c.goods_number, c.goods_attr_id ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_cart') . ' AS c ' . (' WHERE ' . $sess_id . ' AND c.ru_id = \'' . $val . '\' AND c.goods_id = \'' . $g . '\' ORDER BY c.goods_attr_id');
			$res = $GLOBALS['db']->getAll($sql);
			$total_number = 0;
			$total_price = 0;
			...
			if (empty($goods_data['price_model'])) {
				if ($goods_data['moq'] <= $total_number) {
...
function get_wholesale_orders($user_id, $record_count, $page, $where = '', $order = '', $pagesize = 10)
{
	...
	$arr = array();
	$sql = 'SELECT og.ru_id,og.goods_attr ,og.goods_number, oi.inv_type, oi.inv_payee,oi.tax_id, oi.postscript, oi.main_order_id, oi.consignee, oi.order_id,oi.mobile, oi.order_sn, oi.order_status, oi.add_time,oi.pay_id,oi.pay_fee,oi.pay_time,oi.pay_status, ' . '(oi.order_amount) AS total_fee, ' . 'og.goods_id, oi.email, oi.address, oi.province, oi.city, oi.district ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi' . ' left join ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og on oi.order_id = og.order_id' . $left_join . (' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = 0 ') . $where . ' and (select count(*) from ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi2 where oi2.main_order_id = oi.order_id) = 0 ' . 'group by oi.order_id ORDER BY oi.add_time DESC ' . $limit;
	$res = $GLOBALS['db']->query($sql);
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		else if ($row['user_order'] == OS_CONFIRMED) {
			$row['delete_yes'] = 1;
		}
		else {
			$row['delete_yes'] = 0;
		...
		else if ($row['user_order'] == OS_UNCONFIRMED) {
			$order_over = 0;
			$row['handler_order_status'] = false;
			$row['handler'] = '';
		}
		...
		$arr[] = array('order_id' => $row['order_id'], 'order_sn' => $row['order_sn'], 'order_time' => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']), 'sign' => $row['sign'], 'sign' => $shop_information['is_IM'], 'is_dsc' => $row['is_dsc'], 'order_status' => $row['order_status'], 'goods_attr' => $row['goods_attr'], 'status_number' => $status_number, 'pay_name' => $row['pay_name'], 'consignee' => $row['consignee'], 'postscript' => $row['postscript'], 'inv_type' => $row['inv_type'], 'inv_payee' => $row['inv_payee'], 'tax_id' => $row['tax_id'], 'main_order_id' => $row['main_order_id'], 'shop_name' => $row['shop_name'], 'mobile' => $row['mobile'], 'shop_url' => $row['shop_url'], 'order_goods' => $row['order_goods'], 'order_goods_count' => $row['order_goods_count'], 'order_child' => $order_child, 'no_picture' => $GLOBALS['_CFG']['no_picture'], 'order_child' => $order_child, 'delete_yes' => $row['delete_yes'], 'invoice_no' => $row['invoice_no'], 'shipping_name' => $row['shipping_name'], 'pay_name' => $row['pay_name'], 'email' => $row['email'], 'address_detail' => $row['address_detail'], 'address' => $row['address'], 'address_detail' => $address_detail, 'tel' => $row['tel'], 'delivery_time' => $delivery['delivery_time'], 'order_count' => $order_count, 'kf_type' => $basic_info['kf_type'], 'kf_ww' => $kf_ww_one, 'kf_qq' => $kf_qq_one, 'total_fee' => price_format($row['total_fee'], false), 'handler_return' => $row['handler_return'], 'handler' => $row['handler'], 'original_handler' => $row['original_handler'], 'original_handler_return' => $row['original_handler_return'], 'handler_act' => $row['handler_act'], 'handler_return_act' => $row['handler_return_act'], 'return_url' => $row['return_url'], 'remind' => $row['remind'] ? $row['remind'] : '', 'handler_order_status' => $row['handler_order_status'] ? true : false, 'order_over' => $order_over, 'pay_time' => local_date($GLOBALS['_CFG']['time_format'], $row['pay_time']), 'pay_status' => $row['pay_status'], 'pay_name' => $row['pay_name'], 'pay_fee' => $row['pay_fee']);
	}
...
{
	$sql = 'SELECT w.act_id, g.goods_id, g.goods_name, g.goods_thumb, og.goods_number, og.goods_price,og.goods_attr, og.goods_price, og.goods_name AS extension_name, oi.order_sn FROM ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' AS oi ON og.order_id = oi.order_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ON w.goods_id = og.goods_id ' . 'left join ' . $GLOBALS['ecs']->table('goods') . ' as g on og.goods_id = g.goods_id ' . (' WHERE og.order_id = \'' . $order_id . '\'  order by g.goods_id');
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function wholesale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			$sql = 'SELECT w.cat_id, w.cat_name,w.is_show, w.parent_id, w.sort_order, COUNT(s.parent_id) AS has_children ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS w ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS s ON s.parent_id = w.cat_id ' . 'GROUP BY w.cat_id ' . 'ORDER BY w.parent_id, w.sort_order ASC';
			$res = $GLOBALS['db']->getAll($sql);
...
function get_wholesale_child_cat($cat_id = 0, $type = 0)
{
	...
	if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
		$sql = 'SELECT cat_id,cat_name ,parent_id,is_show, style_icon, cat_icon ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ('WHERE parent_id = \'' . $parent_id . '\' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC');
		$res = $GLOBALS['db']->getAll($sql);
		$cat_arr = array();
...
function get_wholesale_child_tree($tree_id = 0, $ru_id = 0)
{
	...
	if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
		$child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ('WHERE parent_id = \'' . $tree_id . '\' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC');
...
function check_wholsale_product_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $goods_model = 0)
{
	...
	if (!empty($product_id)) {
		$sql = 'SELECT w.user_id FROM ' . $GLOBALS['ecs']->table('wholesale_products') . ' AS wp, ' . $GLOBALS['ecs']->table('wholesale') . ' AS w' . (' WHERE wp.goods_id = w.goods_id AND product_id = \'' . $product_id . '\'');
...
function get_user_wholesale_rank($rank_ids)
{
	...
	if ($goods['rank_ids']) {
		$sql = 'SELECT rank_id, rank_name FROM ' . $GLOBALS['ecs']->table('user_rank') . ' WHERE rank_id ' . db_create_in($goods['rank_ids']);
...
{
	$sql = 'SELECT SUM(g.goods_weight * o.goods_number) AS weight, ' . 'SUM(o.goods_price * o.goods_number) AS amount ,' . 'SUM(o.goods_number) AS number ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' AS o, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . ('WHERE o.order_id = \'' . $order_id . '\' ') . 'AND o.goods_id = g.goods_id';
	$row = $GLOBALS['db']->getRow($sql);
