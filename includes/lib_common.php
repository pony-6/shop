...
function get_store_shop_list($libType = 0, $keywords = '', $count = 0, $size = 16, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $area_city = 0, $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '')
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr[$row['shop_id']]['goods_count'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id, $area_city);
		$arr[$row['shop_id']]['goods_list'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id, $area_city, 1);
...
function get_store_shop_goods_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id, $area_city = 0)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		if ($row) {
			...
			$build_uri = array('urid' => $row['user_id'], 'append' => $arr[$row['goods_id']]['shop_name']);
			$domain_url = get_seller_domain_url($row['user_id'], $build_uri);
...
function get_filter_goods_list($filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0, 'mer_ids' => ''), $size = 10, $page = 1, $sort = 'sort_order', $order = 'ASC', $warehouse_id = 0, $area_id = 0, $area_city = 0, $type = '')
{
	...
	$sql = ' SELECT g.goods_id, g.goods_name, g.goods_thumb, g.promote_start_date, g.promote_end_date, g.product_price, g.product_promote_price, ' . $shop_price . ',' . $promote_price . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . (' WHERE 1 ' . $where . ' ' . $where_area . ' GROUP BY g.goods_id ORDER BY ' . $sort . ' ' . $order . ' ');
	$start = ($page - 1) * $size;
