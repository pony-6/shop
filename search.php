...
if ($_REQUEST['act'] == 'advanced_search') {
	...
	if ($search_type == 0) {
		...
		while ($row = $db->FetchRow($res)) {
			...
			$arr[$idx]['sales_volume'] = $row['sales_volume']+$row['sales_volume_base'];
			}else{$arr[$idx]['sales_volume'] = $row['sales_volume'];}
		...
		$smarty->assign('best_goods', get_recommend_goods('best', '', $region_id, $area_info['region_id'], $area_city, 0, 1));
		$smarty->assign('guess_goods', get_guess_goods($_SESSION['user_id'], 1, 1, 7, $region_id, $area_info['region_id'], $area_city));
