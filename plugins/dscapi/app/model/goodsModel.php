...
abstract class goodsModel extends \app\func\common
{
	...
	public function get_where($val = array(), $alias = '')
	{
		...
		$where .= \app\func\base::get_where($val['is_delete'], $alias . 'is_delete');
	...
	public function get_insert($table, $select, $format)
	{
		...
		if ($id) {
			...
			else if ($table == $config['wg']) {
				$info['w_id'] = $id;
			}
			else if ($table == $config['wag']) {
				$info['a_id'] = $id;
			}
			else if ($table == $config['gll']) {
				$info['img_id'] = $id;
			}
			else if ($table == $config['ga']) {
				$info['goods_attr_id'] = $id;
			}
			else if ($table == $config['gtt']) {
				$info['tid'] = $id;
			}
			else if ($table == $config['gtes']) {
				$info['id'] = $id;
			}
			else if ($table == $config['gted']) {
				$info['id'] = $id;
			}
		}

		$common_data = array('result' => 'success', 'msg' => $goodsLang['msg_success']['success'], 'error' => $goodsLang['msg_success']['error'], 'format' => $format, 'info' => $info);
	...
	public function get_more_update($table, $select, $where, $format, $info = array())
	{
		$goodsLang = \languages\goodsLang::lang_goods_update();
		$first_table = $table[0];
		$first_select = $select[0];
	...
	public function get_delete($table, $where, $format)
	{
		...
		if (strlen($where) != 1) {
			...
			$error = \languages\goodsLang::DEL_NULL_PARAM_FAILURE;
		}

		$common_data = array('result' => $return ? 'success' : 'failure', 'msg' => $return ? $goodsLang['msg_success']['success'] : $goodsLang['msg_failure'][$error]['failure'], 'error' => $return ? $goodsLang['msg_success']['error'] : $goodsLang['msg_failure'][$error]['error'], 'format' => $format);
		\app\func\common::common($common_data);
		return \app\func\common::data_back();
	}
						...
						if (!empty($gallery_val['navigateImage']) && $gallery_val['navigateImage'] != 'http://' && (strpos($gallery_val['navigateImage'], 'http://') !== false || strpos($gallery_val['navigateImage'], 'https://') !== false)) {
							if (\app\func\common::get_http_basename($gallery_val['navigateImage'], $admin_temp_dir)) {
								...
								if ($i == 1) {
									...
									$goods_arr = array('original_img' => $img_original, 'goods_img' => $img_url, 'goods_thumb' => $thumb_url, 'goods_sn' => $goods_sn);
									$sql_where = ' goods_id = \'' . $id . '\'';
	...
	public function get_goodsnotification_update($table, $select, $format)
	{
		...
		if (!empty($select)) {
			...
			case 104:
				if (0 < $goods_id) {
					$sql = ' UPDATE ' . $GLOBALS['ecs']->table('goods') . ' SET is_on_sale = \'0\' ,  last_update = \'' . gmtime() . '\', notification = \'成本价变更\' ' . ('WHERE goods_id = \'' . $goods_id . '\' ');
					$GLOBALS['db']->query($sql);
					$sql = ' DELETE FROM ' . $GLOBALS['ecs']->table('cart') . (' WHERE goods_id = \'' . $goods_id . '\' ');
					$GLOBALS['db']->query($sql);
				}
