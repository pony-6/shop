...
abstract class orderModel extends \app\func\common
{
	...
	{
		$this->alias_config = array('order_info' => 'o', 'order_goods' => 'og');
	...
	{
		$method = array('dsc.order.goods.list.get', 'dsc.order.goods.info.get', 'dsc.order.goods.update.post', 'dsc.order.goods.del.get');
		$where = 1;
		$conditions = '';
	...
	public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order, $alias = '')
	{
		...
		if ($result['list']) {
			foreach ($result['list'] as $key => $row) {
				$country = \app\func\common::get_regions_info($row['country']);
				$province = \app\func\common::get_regions_info($row['province']);
				$city = \app\func\common::get_regions_info($row['city']);
				$district = \app\func\common::get_regions_info($row['district']);
				$street = \app\func\common::get_regions_info($row['street']);
	...
	public function get_insert($table, $select, $format)
	{
		...
		if ($id) {
			...
			else if ($table == $config['og']) {
				$info['rec_id'] = $id;
			}
		}

		$common_data = array('result' => 'success', 'msg' => $orderLang['msg_success']['success'], 'error' => $orderLang['msg_success']['error'], 'format' => $format, 'info' => $info);
	...
	public function get_delete($table, $where, $format)
	{
		...
		if (strlen($where) != 1) {
			...
			$error = \languages\orderLang::DEL_NULL_PARAM_FAILURE;
		}

		$common_data = array('result' => $return ? 'success' : 'failure', 'msg' => $return ? $orderLang['msg_success']['success'] : $orderLang['msg_failure'][$error]['failure'], 'error' => $return ? $orderLang['msg_success']['error'] : $orderLang['msg_failure'][$error]['error'], 'format' => $format);
		\app\func\common::common($common_data);
		return \app\func\common::data_back();
	}
	...
	public function get_confirmorder($table, $order_select, $format)
	{
		...
		if (!empty($order_select)) {
			...
			if (empty($shipping_info)) {
				$shipping_info = array('shipping_id' => 0, 'shipping_name' => $order_select['expressName']);
			}
