...
class goods extends \app\model\goodsModel
{
	...
	{
		$this->goods($where);
		$this->wehre_val = array('seller_id' => $this->seller_id, 'brand_id' => $this->brand_id, 'cat_id' => $this->cat_id, 'user_cat' => $this->user_cat, 'goods_id' => $this->goods_id, 'goods_sn' => $this->goods_sn, 'is_delete' => $this->is_delete, 'bar_code' => $this->bar_code, 'w_id' => $this->w_id, 'a_id' => $this->a_id, 'region_id' => $this->region_id, 'region_sn' => $this->region_sn, 'img_id' => $this->img_id, 'attr_id' => $this->attr_id, 'goods_attr_id' => $this->goods_attr_id, 'tid' => $this->tid, 'seller_type' => $this->seller_type);
			...
			if (!isset($select['w_id'])) {
				if (strlen($this->where) != 1) {
					...
					if (!$info) {
						...
						$goods_sn = 0;
						if (isset($select['goods_sn']) && !empty($select['goods_sn'])) {
							$where = 'goods_sn = \'' . $select['goods_sn'] . '\' AND w_id <> \'' . $info['w_id'] . '\' AND user_id = \'' . $info['user_id'] . '\'';
							$goods_sn = $this->get_select_info($this->table, '*', $where);
						}
			...
			if (!isset($select['a_id'])) {
				if (strlen($this->where) != 1) {
					...
					if (!$info) {
						...
						$goods_sn = 0;
						if (isset($select['goods_sn']) && !empty($select['goods_sn'])) {
							$where = 'goods_sn = \'' . $select['goods_sn'] . '\' AND a_id <> \'' . $info['a_id'] . '\' AND user_id = \'' . $info['user_id'] . '\'';
							$goods_sn = $this->get_select_info($this->table, '*', $where);
						}
