	...
	public function actionStoreIn()
	{
		if (IS_POST) {
			...
			foreach ($store as $key => $value) {
				...
				$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' where user_id = \'' . $value['user_id'] . '\' AND is_on_sale = 1 AND is_delete = 0 AND is_alone_sale = 1 AND review_status > 2 AND is_promote = 1 AND promote_start_date < \'' . $time . '\' AND promote_end_date > \'' . $time . '\' ';
