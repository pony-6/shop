...
function get_brands_list($cat = 0, $keyword = '', $size = 10, $page = 0)
{
	...
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('brand') . ' as b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' WHERE ' . $where . ' AND b.is_show = 1 ORDER BY b.sort_order ASC';
	$total = $GLOBALS['db']->getOne($sql);
	$sql = 'SELECT b.brand_id, b.brand_name, b.brand_logo, b.index_img, COUNT(*) AS goods_num, IF(b.brand_logo > \'\', \'1\', \'0\') AS tag ' . ' FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' WHERE ' . $where . ' AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC';
	$res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
	$list = array();
	...
	foreach ($res as $row) {
		...
		$list[$row['brand_id']]['url'] = url('brand/index/detail', array('id' => $row['brand_id']));
