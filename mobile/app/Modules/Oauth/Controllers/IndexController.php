...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	{
		$type = I('get.type');
		$refer = I('get.refer', '', array('htmlspecialchars', 'trim'));
	...
	public function actionBindRegister()
	{
		if (IS_POST) {
			...
			$type = input('type', '', array('htmlspecialchars', 'trim'));
	...
	protected function doRegister($res, $type = '')
	{
		...
		if (is_dir(APP_WECHAT_PATH)) {
			$wechat_user = dao('wechat_user')->field('parent_id')->where(array('unionid' => $res['unionid']))->find();
