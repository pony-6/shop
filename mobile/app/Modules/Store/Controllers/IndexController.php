...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	'totalPage' => 0
			...
			if ($store === false) {
				...
				if ($store_list) {
					foreach ($store_list as $key) {
						...
						$store[] = array('shop_id' => $key['shop_id'], 'url' => build_uri('store', array('stid' => $key['user_id'])), 'user_id' => $key['user_id'], 'shop_name' => get_shop_name($key['user_id'], 1), 'self_run' => $key['self_run'], 'shop_logo' => get_image_path(ltrim($key['logo_thumb'], '../')), 'commentrank' => $merchants_goods_comment['cmt']['commentRank']['zconments']['score'], 'commentrank_bg' => $this->cmt($merchants_goods_comment['cmt']['commentRank']['zconments']['score']), 'commentserver' => $merchants_goods_comment['cmt']['commentServer']['zconments']['score'], 'commentserver_bg' => $this->cmt($merchants_goods_comment['cmt']['commentServer']['zconments']['score']), 'commentdelivery' => $merchants_goods_comment['cmt']['commentDelivery']['zconments']['score'], 'commentdelivery_bg' => $this->cmt($merchants_goods_comment['cmt']['commentDelivery']['zconments']['score']), 'commentrank_font' => $this->font($merchants_goods_comment['cmt']['commentRank']['zconments']['score']), 'commentrank_box' => $this->boxbg($merchants_goods_comment['cmt']['commentRank']['zconments']['score']), 'commentserver_font' => $this->font($merchants_goods_comment['cmt']['commentServer']['zconments']['score']), 'commentserver_box' => $this->boxbg($merchants_goods_comment['cmt']['commentServer']['zconments']['score']), 'commentdelivery_font' => $this->font($merchants_goods_comment['cmt']['commentDelivery']['zconments']['score']), 'commentdelivery_box' => $this->boxbg($merchants_goods_comment['cmt']['commentDelivery']['zconments']['score']), 'gaze_number' => $gaze, 'gaze_status' => $status, 'goods' => $goodsarr, 'title' => $title = 0 < count($goods) ? '爆款商品' : '', 'distance' => $distance);
					}
				}
			...
			$list = array('list' => $store, 'totalPage' => ceil($counts / $offset));
	...
	protected function shopdata($data = array())
	{
		...
		$info['count_goods_promote'] = $this->sql('is_promote = 1 AND user_id=' . $user_id . '   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 AND promote_start_date < \'' . $time . '\' AND promote_end_date > \'' . $time . '\' ' . $this->review_goods);
