...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		if ($goods['consumption']) {
			...
			$consumption = array('act_name' => rtrim($consumption, ','), 'url' => '#', 'type' => 'consumption');
		}
		...
		if (!empty($default_attr_id)) {
			$default_attr_id = !empty($default_attr_id) ? explode(',', $default_attr_id) : array();
		...
		if (0 < $group_count) {
			...
			$fittings_list = get_goods_fittings(array($this->goods_id), $this->region_id, $this->area_id);
			...
			$this->assign('fittings_tab_index', $fittings_index);
			$fittings_list = get_array_sort($fittings_list, 'spare_price_ori', 'DESC');
			$fittings_list = array_values($fittings_list);
		...
		if (!empty($_COOKIE['ECS']['history_goods'])) {
			...
			while (C('shop.history_number') < count($his)) {
				array_pop($his);
			}
		...
		$goods_region = array('country' => 1, 'province' => $this->province_id, 'city' => $this->city_id, 'district' => $this->district_id, 'town_region_id' => $this->town_region_id);
		...
		if (0 < $goods['user_id']) {
			...
			$merchants_goods_comment['commentRank'] = comment_font($merchants_goods_comment['cmt']['commentRank']['zconments']['goodReview']);
	...
	public function actionSharing()
	{
		if (IS_AJAX) {
			...
			$goods_price = input('price', '', array('trim', 'html_in'));
	...
	public function actionPrice()
	{
		...
		$attr = I('attr', '', array('htmlspecialchars', 'trim'));
		...
		$area_city = I('area_city', 0, 'intval');
	...
	{
		if (IS_AJAX) {
			$res = array('err_msg' => '', 'result' => '', 'number' => 1);
			$goods_id = input('id', 0, 'intval');
			$type = input('type', 0, 'intval');
			$onload = input('onload', '', array('htmlspecialchars', 'trim'));
			...
			$parent_id = intval($group->fittings_goods);
			...
			if ($type == 1 && $onload != 'onload' || $type == 0) {
				...
				$cart_combo_data = array('goods_attr_id' => implode(',', $attr_id), 'product_id' => $product_info['product_id'], 'goods_attr' => addslashes($goods_attr), 'goods_price' => $goods_price);
