...
class RefoundController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		if (IS_AJAX) {
			$order_list = get_all_return_order($order_id, $size, $page);
	...
	public function actionList()
	{
		...
		if (IS_AJAX) {
			$refound_list = return_order($order_id, $size, $page);
			exit(json_encode(array('refound_list' => $refound_list['list'], 'totalPage' => $refound_list['totalpage'])));
		}
	...
	public function actionSubmitReturn()
	{
		...
		if ($query) {
			...
			show_message(L('Apply_Success_Prompt'), L('See_returnlist'), url('detail', array('ret_id' => $return['ret_id'])), 'success', true);
		}
		else {
			show_message(L('Apply_Abnormal'), '', '', 'error', true);
		}
