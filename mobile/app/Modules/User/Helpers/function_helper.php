...
{
	$time = gmtime();
	$sql = 'SELECT count(u.bonus_id) as bonus_count FROM {pre}user_bonus AS u ' . ' LEFT JOIN {pre}bonus_type AS b ON u.bonus_type_id=b.type_id ' . (' WHERE u.user_id = \'' . $user_id . '\' AND b.use_end_date > ' . $time . ' and u.order_id = 0 ');
	$res = $GLOBALS['db']->getRow($sql);
...
function get_all_return_order($order_id = 0, $size = 10, $page = 1)
{
	...
	$sql = 'SELECT count(o.order_id) as num FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' ' . ' AND o.order_status IN (\'' . OS_CONFIRMED . '\',\'' . OS_SPLITED . '\', \'' . OS_SPLITING_PART . '\', \'' . OS_RETURNED_PART . '\', \'' . OS_ONLY_REFOUND . '\') ' . ' AND o.pay_status IN (\'' . PS_PAYED . '\',\'' . PS_PAYED_PART . '\',\'' . PS_REFOUND_PART . '\') ' . ' AND o.is_delete = 0 AND o.pay_time > ' . $log_time . ' AND o.pay_time < ' . $time . $where . ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ' . ' ';
	$total = $GLOBALS['db']->getOne($sql);
	$sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.extension_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' ' . ' AND o.order_status IN (\'' . OS_CONFIRMED . '\',\'' . OS_SPLITED . '\', \'' . OS_SPLITING_PART . '\', \'' . OS_RETURNED_PART . '\', \'' . OS_ONLY_REFOUND . '\') ' . ' AND o.pay_status IN (\'' . PS_PAYED . '\',\'' . PS_PAYED_PART . '\',\'' . PS_REFOUND_PART . '\') ' . ' AND o.is_delete = 0 AND o.pay_time > ' . $log_time . ' AND o.pay_time < ' . $time . $where . ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ' . ' group by o.order_id ORDER BY o.add_time DESC';
	...
	return array('list' => $order, 'totalpage' => ceil($total / $size));
}
...
function get_user_accountlog_count($user_id = 0, $account_type = '', $page = 1, $size = 10)
{
	...
	return array('list' => $account_log, 'totalpage' => ceil($total / $size));
}
