...
{
	$where = '';
	$total_arr = $GLOBALS['db']->getAll('SELECT oi.order_id FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi' . ' left join ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og on oi.order_id = og.order_id' . (' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = \'0\' ') . ' and (select count(*) from ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi2 where oi2.main_order_id = oi.order_id) = 0 ' . ' group by oi.order_id ORDER BY oi.add_time DESC');
	$total = is_array($total_arr) ? count($total_arr) : 0;
	$start = ($page - 1) * $num;
	$arr = array();
	$sql = 'SELECT og.ru_id, oi.order_id,oi.main_order_id, oi.order_sn, oi.consignee,oi.order_status,oi.address,oi.mobile,oi.order_amount, oi.add_time,oi.postscript, oi.is_delete, oi.inv_payee, oi.inv_type, oi.invoice_type, oi.vat_id, oi.tax_id,oi.extension_code,oi.pay_id ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi' . ' left join ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og on oi.order_id = og.order_id' . (' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = \'0\' ') . ' and (select count(*) from ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi2 where oi2.main_order_id = oi.order_id) = 0 ' . (' group by oi.order_id ORDER BY oi.add_time DESC LIMIT ' . $start . ', ' . $num);
	$res = $GLOBALS['db']->query($sql);

	foreach ($res as $key => $row) {
		$row['order_goods'] = get_purchase_order_goods($row['order_id']);
		$order_id = $row['order_id'];
		$pay = dao('payment')->where(array('pay_id' => $row['pay_id']))->find();
		$ru_id = $row['ru_id'];
		...
		$arr[] = array('order_id' => $row['order_id'], 'order_sn' => $row['order_sn'], 'order_time' => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']), 'order_status' => $row['order_status'], 'order_del' => $row['order_del'], 'order_amount' => price_format($row['order_amount'], false), 'status' => $row['status'], 'consignee' => $row['consignee'], 'mobile' => $row['mobile'], 'main_order_id' => $row['main_order_id'], 'user_name' => get_shop_name($ru_id, 1), 'order_goods' => $row['order_goods'], 'order_goods_num' => count($row['order_goods']), 'no_picture' => $GLOBALS['_CFG']['no_picture'], 'delete_yes' => $row['delete_yes'], 'invoice_no' => $row['invoice_no'], 'address_detail' => $row['address_detail'], 'address' => $row['address'], 'postscript' => $row['postscript'], 'address_detail' => $address_detail, 'tel' => $row['mobile'], 'order_count' => $order_count, 'handler_return' => $row['handler_return'], 'pay_name' => $pay['pay_name'], 'pay_status' => $row['pay_status'], 'inv_payee' => $row['inv_payee'], 'inv_type' => $row['inv_type'], 'tax_id' => $row['tax_id'], 'invoice_type' => $row['invoice_type'], 'handler' => $row['handler'], 'extension_code' => $row['extension_code']);
	}
...
function get_purchase_order_goods($order_id = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		$arr[$key]['url'] = url('purchase/index/goods', array('id' => $row['act_id']));
	}
