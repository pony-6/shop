...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionIndex()
	{
		...
		$banners = \App\Modules\Purchase\Models\Purchase::get_banner(256, 3);
	...
	public function actionList()
	{
		...
		$page = 0 < $page ? $page : 1;
		$size = !empty($GLOBALS['_CFG']['page_size']) && 0 < intval($GLOBALS['_CFG']['page_size']) ? intval($GLOBALS['_CFG']['page_size']) : 10;
		$this->assign('page_title', '批发列表');
		$this->assign('action', 'list');
		$cat_id = I('id', 0, 'intval');

		if ($cat_id) {
			$this->assign('cat_name', \App\Modules\Purchase\Models\Purchase::getCatName($cat_id));
		}
	...
	public function actionGoodsList()
	{
		...
		$result = \App\Modules\Purchase\Models\Purchase::get_wholesale_list($act_id, $size, $page);
	...
	public function actionAsyncSearchList()
	{
		...
		$size = !empty($GLOBALS['_CFG']['page_size']) && 0 < intval($GLOBALS['_CFG']['page_size']) ? intval($GLOBALS['_CFG']['page_size']) : 10;
		$list = \App\Modules\Purchase\Models\Purchase::get_search_goods_list($_REQUEST['keywords'], $page, $size);
	...
	public function actionGoods()
	{
		...
		$goods = \App\Modules\Purchase\Models\Purchase::get_wholesale_goods_info($act_id);
		...
		foreach ($goods['volume_price'] as $list) {
			if ($min == 0 || $list['volume_number'] < $min) {
				$min = $list['volume_number'];
			}
		}

		$this->assign('min', $min);
		$properties = \App\Modules\Purchase\Models\Purchase::get_wholesale_goods_properties($goods['goods_id'], $region_id, $area_id);
		$this->assign('specification', $properties['spe']);
		$main_attr_list = \App\Modules\Purchase\Models\Purchase::get_wholesale_main_attr_list($goods['goods_id']);
		$this->assign('main_attr_list', $main_attr_list);
		$this->assign('properties', $properties['pro']);
		$is_jurisdiction = \App\Modules\Purchase\Models\Purchase::isJurisdiction($goods);
		$this->assign('is_jurisdiction', 1);
		$cartInfo = \App\Modules\Purchase\Models\Purchase::get_wholesale_cart_info();
		$this->assign('cart_number', $cartInfo['number']);
		$back_url = url('user/login/index', array('back_act' => urlencode(__SELF__)));
	...
	public function actionAddToCart()
	{
		...
		$rank_ids = get_table_date('wholesale', 'goods_id=\'' . $goods_id . '\'', array('rank_ids'), 2);
		$is_jurisdiction = 0;
		...
		if (0 < $_SESSION['user_id']) {
			...
			$result['error'] = 2;
			$result['content'] = url('user/login/index', array('back_act' => urlencode($back_act)));
		...
		$goods_data = \App\Modules\Purchase\Models\Purchase::get_count_cart();
	...
	public function actionDown()
	{
		...
		$log_id = $this->insert_pay_log($main_order_id, $order_amount, PAY_WHOLESALE);
		$sql = ' DELETE FROM ' . $GLOBALS['ecs']->table('wholesale_cart') . $where;
		$GLOBALS['db']->query($sql);
		$result = array('code' => 0, 'message' => '提交成功');
		$this->ajaxReturn($result);
	}
	...
	public function actionCart()
	{
		...
		$goods_data = \App\Modules\Purchase\Models\Purchase::wholesale_cart_goods($goods_id, $rec_ids);
	...
	public function actionUpdateCartGoods()
	{
		...
		$GLOBALS['db']->query($sql);
		$cart_goods = \App\Modules\Purchase\Models\Purchase::wholesale_cart_goods(0, $rec_ids);
		$goods_list = array();
		...
		$cart_info = \App\Modules\Purchase\Models\Purchase::wholesale_cart_info(0, $rec_ids);
		$result['cart_info'] = $cart_info;
		$result['goods'] = \App\Modules\Purchase\Models\Purchase::cartInfo($rec_id);
		$this->ajaxReturn($result);
	}
	...
	public function actionShow()
	{
		...
		if (IS_AJAX) {
			$purchase_list = \App\Modules\Purchase\Models\Purchase::get_purchase_list($filter_array, $size, $page);
			exit(json_encode(array('list' => array_values($purchase_list['purchase_list']), 'totalPage' => $purchase_list['page_count'])));
		}
	...
	public function actionShowDetail()
	{
		...
		$purchase_info = \App\Modules\Purchase\Models\Purchase::get_purchase_info($purchase_id);
