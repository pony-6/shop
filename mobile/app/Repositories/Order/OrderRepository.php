...
class OrderRepository
{
	...
	{
		$goods = \App\Models\OrderGoods::where('order_id', $orderId)->select('goods.goods_thumb', 'order_goods.goods_price', 'order_goods.goods_number', 'order_goods.goods_id', 'order_goods.goods_name', 'order_goods.goods_sn', 'order_goods.ru_id', 'order_goods.drp_money')->join('goods', 'goods.goods_id', '=', 'order_goods.goods_id')->get();
	...
	{
		return \App\Models\OrderAction::insertGetId($params);
	}

	public function updateOrderInfo($order_id, $params)
	{
		\App\Models\OrderInfo::where('order_id', $order_id)->update($params);
	}

	public function orderMoney($uid)
	{
		$res = \App\Models\OrderInfo::where('user_id', $uid)->where('pay_status', 2)->sum('goods_amount');
		return $res;
	}
	...
	public function integral_to_give($order)
	{
		...
		$sql = 'SELECT SUM(og.goods_number * IF(g.give_integral > -1, g.give_integral, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points ' . 'FROM ' . $prefix . 'order_goods AS og, ' . $prefix . 'goods AS g ' . 'WHERE og.goods_id = g.goods_id ' . ('AND og.order_id = \'' . $order . '\' ') . 'AND og.goods_id > 0 ' . 'AND og.parent_id = 0 ' . 'AND og.is_gift = 0 AND og.extension_code != \'package_buy\'';
