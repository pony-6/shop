...
class GoodsService
{
	...
	public function getPromotionInfo($goods_id = '', $ru_id = 0)
	{
		...
		if (empty($goods_id)) {
			foreach ($res as $rows) {
				...
				$favourable[$rows['act_id']]['url'] = url('activity/index/detail', array('id' => $rows['act_id']));
	...
	public function goodsShare($id, $uid, $path = '', $width = 430, $type = 'goods')
	{
		...
		$user = array('name' => isset($userInfo['nick_name']) ? $userInfo['nick_name'] : $userInfo['user_name'], 'id' => $userInfo['id'], 'pic' => get_image_path($userInfo['user_picture']), 'shop_name' => $shop_name, 'image_name' => $image_name);
