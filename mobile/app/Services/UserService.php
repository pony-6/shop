...
class UserService
{
	...
	public function orderList($args)
	{
		...
		foreach ($orderList as $k => $v) {
			...
			$orderList[$k]['total_amount'] = $v['order_amount'];
	...
	{
		$action_log = array('order_id' => $order_id, 'action_user' => $action_user, 'order_status' => $order_status, 'shipping_status' => $shipping_status, 'pay_status' => $pay_status, 'action_note' => $action_note, 'log_time' => gmtime());
		$this->orderRepository->addOrderAction($action_log);
		$order_info = array('order_status' => $order_status, 'shipping_status' => $shipping_status, 'pay_status' => $pay_status);
