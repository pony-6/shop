...
class FlowService
{
	...
	private function arrangeCartGoods($userId, $flow_type)
	{
		...
		$discount = price_format($cartGoodsList['total']['discount'], false);
		...
		sort($list);
		return array('list' => $list, 'order_total' => $totalAmount, 'order_total_formated' => price_format($totalAmount, false), 'discount' => $discount);
	}
	...
	public function submitOrder($args)
	{
		...
		$parent_id = $this->drpRepository->drpUserShop($userId);
	...
	private function sendCloudOrderGoods($cart_goods = array(), $order = array())
	{
		...
		if (!empty($order_detaillist)) {
			...
			$order_request['area'] = $this->regionRepository->getRegionName($order['district']);
			$order_request['city'] = $this->regionRepository->getRegionName($order['city']);
			$order_request['province'] = $this->regionRepository->getRegionName($order['province']);
	...
	public function orderDetail($args)
	{
		...
		$list = array('order_amount' => price_format($main_order['order_amount'], false), 'order_amount_formated' => price_format($main_order['order_amount'], false), 'order_sn' => $son_order);
		return $list;
	}
}
