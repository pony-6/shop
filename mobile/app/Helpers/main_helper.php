...
function get_user_info($id = 0)
{
	...
	$sql = 'SELECT u.user_id, u.email, u.mobile_phone, u.user_name, u.nick_name, u.user_money, u.pay_points' . ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' . (' WHERE u.user_id = \'' . $id . '\'');
...
function cart_number($type = CART_GENERAL_GOODS)
{
	...
	$sql = 'SELECT SUM(goods_number) AS number ' . ' FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND rec_type = \'' . $type . '\' AND stages_qishu=\'-1\' AND store_id = 0';
	$row = $GLOBALS['db']->GetRow($sql);
