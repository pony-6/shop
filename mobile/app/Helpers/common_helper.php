...
function get_promotion_info($goods_id = '', $ru_id = 0)
{
	...
	foreach ($res as $data) {
		...
		case GAT_SNATCH:
			...
			$snatch[$data['act_id']]['url'] = url('snatch/index/detail', array('id' => $data['act_id']));
		...
		case GAT_GROUP_BUY:
			...
			$group[$data['act_id']]['url'] = url('groupbuy/index/detail', array('id' => $data['act_id']));
		...
		case GAT_AUCTION:
			...
			$auction[$data['act_id']]['url'] = url('auction/index/detail', array('id' => $data['act_id']));
...
function get_goods_sale($order_id = 0, $order = array())
{
	...
	if (C('shop.sales_volume_time') == SALES_PAY && $order['pay_status'] == PS_PAYED) {
		...
		if (C('shop.sales_volume_time') == SALES_SHIP && $order['shipping_status'] == SS_SHIPPED) {
			$is_volume = 1;
		}
	}
