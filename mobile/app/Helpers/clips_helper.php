...
function get_collection_goods($user_id, $record_count, $limit = '')
{
	...
	$leftJoin = '';
	$other = array('province_id' => $province_id, 'city_id' => $city_id);
...
{
	$sql = 'SELECT COUNT(*) FROM {pre}user_account  WHERE user_id = \'' . $user_id . '\'  AND process_type ' . db_create_in(array(SURPLUS_SAVE, SURPLUS_RETURN));
	...
	return array('log_list' => $account_log, 'totalpage' => ceil($total / $size));
}
...
function get_account_log_info($user_id, $log_id = 0)
{
	...
	if (!empty($log_id)) {
		$sql = 'SELECT ua.*, uaf.bank_number FROM ' . $GLOBALS['ecs']->table('user_account') . ' AS ua' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('user_account_fields') . ' AS uaf ' . ' ON ua.id = uaf.account_id ' . (' WHERE ua.user_id = \'' . $user_id . '\' AND ua.id=\'' . $log_id . '\'');
		...
		if ($rows['is_paid'] == 0 && $rows['process_type'] == 0) {
			$rows['handle'] = '<a class="btn-submit box-flex" href="' . url('user/account/pay', array('id' => $rows['id'], 'pid' => $pay_id)) . '">' . L('pay') . '</a>';
		}
	}
...
{
	$sql = 'SELECT count(*) as num FROM ' . $GLOBALS['ecs']->table('value_card') . ' AS v ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('value_card_type') . ' AS t ON v.tid = t.id ' . (' WHERE v.user_id = \'' . $user_id . '\' order by v.vid DESC ');
	$total = $GLOBALS['db']->getOne($sql);
...
{
	$sql = 'SELECT count(*) as num FROM ' . $GLOBALS['ecs']->table('value_card_record') . ' AS r ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON r.order_id = o.order_id ' . (' WHERE r.vc_id = \'' . $vc_id . '\' order by r.rid DESC ');
	$total = $GLOBALS['db']->getOne($sql);
