...
function get_user_bouns_list($user_id, $type = 0, $size = 10, $page = 1)
{
	...
	$sql = 'SELECT u.bonus_sn, u.order_id, b.user_id as ru_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date, b.usebonus_type ' . ' FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b ' . ' WHERE  u.bonus_type_id = b.type_id ' . $where . ' AND u.user_id = \'' . $user_id . '\'';
	...
	return array('list' => $arr, 'totalpage' => ceil($total / $size));
}
...
function get_user_conut_bonus($user_id, $type = 0)
{
	...
	$sql = 'SELECT  count(*) as num ' . ' FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b ' . ' WHERE  u.bonus_type_id = b.type_id ' . $where . ' AND u.user_id = \'' . $user_id . '\'';
