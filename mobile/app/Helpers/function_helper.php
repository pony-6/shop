...
function get_first_cate_list($parent_id = 0, $type = 0, $catarr = array(), $user_id = 0)
{
	if ($type == 1) {
		...
		$sql = 'SELECT cat_id, cat_name, cat_alias_name FROM ' . $GLOBALS['ecs']->table('category') . (' WHERE parent_id = \'' . $parent_id . '\'');
		return $GLOBALS['db']->getAll($sql);
	}
}
...
function get_add_childCategory_info($cat_id, $user_id)
{
	...
	foreach ($res as $key => $row) {
		...
		$parents = dao('category')->field('cat_name, cat_alias_name')->where(array('cat_id' => $row['parent_id']))->find();
		$arr[$key]['parent_name'] = !empty($parents['cat_alias_name']) ? $parents['cat_alias_name'] : $parents['cat_name'];
		$parent = array('user_id' => $user_id, 'cat_id' => $row['cat_id'], 'parent_id' => $row['parent_id'], 'cat_name' => $arr[$key]['cat_name'], 'parent_name' => $arr[$key]['parent_name']);
...
function get_insert_order_goods_single($orderInfo, $row, $order_id, $ru_number)
{
	...
	$shipping_dateStr = empty($row['shipping_datestr']) ? '' : $row['shipping_datestr'];
...
function get_seller_postscript($ru_id = 0, $postscript_arr = '')
{
	...
	return array('postscript' => $new_postscript);
}
...
function get_seller_cac_order($ru_id, $point_id, $shipping_dateStr)
{
	...
	return array('point_id' => $pointId, 'shipping_dateStr' => $dateStr);
}
