...
function get_cart_combo_goods_list($goods_id = 0, $parent = 0, $group_id = '')
{
	...
	$sql = 'SELECT goods_price, goods_number, goods_id FROM ' . $GLOBALS['ecs']->table('cart_combo') . ' WHERE ' . $sess_id . (' AND (parent_id = \'' . $parent . '\' OR (goods_id = \'' . $parent . '\' AND parent_id = \'0\')) AND group_id = \'' . $group_id . '\'');
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	$arr['combo_amount'] = 0;
	$arr['combo_number'] = 0;
...
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
	...
	if (empty($history)) {
		...
		$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.model_attr, g.market_price, ' . ('IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\'), g.shop_price * \'' . $_SESSION['discount'] . '\')  AS shop_price, ') . 'IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price,' . ' g.sales_volume, g.user_id, g.promote_start_date, g.promote_end_date, g.product_price, g.product_promote_price FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . ('ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ') . (' WHERE ' . $where_cat . ' ' . $goods_str . ' AND g.is_on_sale = 1 AND g.is_hot = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.sales_volume > 0 ' . $tag_where . ' LIMIT ' . $start . ', ' . $limit);
		$query = $GLOBALS['db']->query($sql);
	}
