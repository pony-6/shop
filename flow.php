				...
				if ($favourable_activity['act_range'] == FAR_CATEGORY && $row1['extension_code'] != 'package_buy') {
					...
					if (in_array(trim($cat_id), $id_list) && $row1['is_gift'] == 0 || $row1['is_gift'] == $favourable_activity['act_id']) {
						if ($row1) {
							...
							if ($row1['is_gift'] == 0 && in_array($cat_id, $fav_act_range_ext) && $favourable_activity['act_id'] == $row1['act_id']) {
								$favourable_box['act_id'] = $favourable_activity['act_id'];
								$favourable_box['act_name'] = $favourable_activity['act_name'];
								$favourable_box['act_type'] = $favourable_activity['act_type'];
				...
				if ($favourable_activity['act_range'] == FAR_BRAND && $row1['extension_code'] != 'package_buy') {
					...
					if (in_array(trim($brand_id), $get_act_range_ext) && $row1['is_gift'] == 0 || $row1['is_gift'] == $favourable_activity['act_id']) {
						if ($row1) {
							...
							if ($row1['is_gift'] == 0 && strstr($act_range_ext_str, trim($brand_id_str)) && $favourable_activity['act_id'] == $row1['act_id']) {
								$favourable_box['act_id'] = $favourable_activity['act_id'];
								$favourable_box['act_name'] = $favourable_activity['act_name'];
								$favourable_box['act_type'] = $favourable_activity['act_type'];
...
if ($_REQUEST['step'] == 'add_to_cart') {
	...
	if (!is_numeric($goods->number) || intval($goods->number) <= 0) {
		...
		if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $warehouse_id, $area_id, $area_city, $goods->stages_qishu, $goods->store_id, $goods->end_time, $goods->store_mobile)) {
			if (!empty($goods->divId)) {
				$result['divId'] = $goods->divId;
			}
	...
	if ($res) {
		...
		$GLOBALS['db']->query($sql);
		$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('cart') . '(' . 'user_id, session_id, goods_id, goods_sn, product_id, group_id, goods_name, market_price, goods_price, goods_number, goods_attr, is_real, ' . 'extension_code, parent_id, rec_type, is_gift, is_shipping, can_handsel, model_attr, goods_attr_id, warehouse_id, area_id, area_city, add_time' . ')' . ' SELECT ' . 'user_id, session_id, goods_id, goods_sn, product_id, group_id, goods_name, market_price, goods_price, goods_number, goods_attr, is_real, ' . 'extension_code, parent_id, rec_type, is_gift, is_shipping, can_handsel, model_attr, goods_attr_id, warehouse_id, area_id, area_city, add_time' . ' FROM ' . $GLOBALS['ecs']->table('cart_combo') . ' WHERE ' . $sess_id . ' AND group_id = \'' . $group . '\'';
		$GLOBALS['db']->query($sql);
...
if ($_REQUEST['step'] == 'add_cart_combo_goodsAttr') {
	...
	$cart_goods_list = cart_goods($flow_type, $cart_value, 1, $region_id, $area_info['region_id'], $area_city['region_id'], $consignee, $store_id);
	...
	if ((!isset($_CFG['use_integral']) || $_CFG['use_integral'] == '1') && 0 < $_SESSION['user_id'] && 0 < $user_info['pay_points'] && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
		...
		$smarty->assign('order_max_integral', flow_available_points($cart_value, $region_id, $area_id, $area_city));
	...
	$tmp_shipping_id_arr = $json->decode($_POST['shipping_id']);
	$order = flow_order_info();
	$flow_points = flow_available_points($_SESSION['cart_value'], $region_id, $area_id, $area_city);
	...
	$user_info = user_info($_SESSION['user_id']);
	$_SESSION['cart_value'] = $cart_value;
	$flow_points = flow_available_points($cart_value, $region_id, $area_id, $area_city);
	$user_points = $user_info['pay_points'];
	...
	if ($_REQUEST['act'] == 'balance') {
		...
		if (0 < $user_id) {
			...
			$flow_points = flow_available_points($done_cart_value, $region_id, $area_id, $area_city);
			$user_points = $user_info['pay_points'];
		...
		if (0 < $order['bonus_id']) {
			...
			if (empty($bonus) || $bonus['user_id'] != $user_id || 0 < $bonus['order_id'] || cart_amount(true, $flow_type, $done_cart_value) < $bonus['min_goods_amount']) {
				$order['bonus_id'] = 0;
			}
		}
			...
			$now = gmtime();
			if (empty($bonus) || 0 < $bonus['user_id'] || 0 < $bonus['order_id'] || cart_amount(true, $flow_type, $done_cart_value) < $bonus['min_goods_amount'] || $bonus['use_end_date'] < $now) {
			}
			else {
				if (0 < $user_id) {
		...
		if (0 < $order['uc_id']) {
			...
			if (empty($coupons) || $coupons['user_id'] != $user_id || $coupons['is_use'] == 1 || cart_amount(true, $flow_type, $done_cart_value) < $coupons['cou_man']) {
				$order['uc_id'] = 0;
			}
		}
	...
	if (isset($_POST['rec_id']) && isset($_POST['goods_number'])) {
		...
		if (0 < $val) {
			...
			if ($row['extension_code'] == 'package_buy') {
				...
				$goods_price = get_final_price($row['goods_id'], $val, true, $attr_id, $_POST['warehouse_id'], $_POST['area_id'], $area_city, 0, 0, $add_tocart);
				$set = '';
	...
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
	$cart_goods = cart_goods($flow_type, $rec_id, 0, $region_id, $area_id, $area_city);
...
else if ($_REQUEST['step'] == 'cart_change_fav') {
	require_once ROOT_PATH . 'includes/cls_json.php';
	$json = new JSON();
	$result = array('error' => 0, 'message' => '');
	get_request_filter();
	$act_id = !empty($_REQUEST['aid']) ? intval($_REQUEST['aid']) : 0;
	$goods_id = !empty($_REQUEST['gid']) ? intval($_REQUEST['gid']) : 0;
	$rec_id = !empty($_REQUEST['rid']) ? intval($_REQUEST['rid']) : 0;
	$user_id = !empty($_SESSION['user_id']) ? $_SESSION['user_id'] : 0;
	...
	$GLOBALS['db']->query($sql);
	update_cart_goods_fav($rec_id, $user_id, $act_id);
	$smarty->assign('area_id', $area_id);
	$smarty->assign('flow_region', $_COOKIE['flow_region']);
	...
	$guess_goods = get_guess_goods($_SESSION['user_id'], 1, 1, 18, $region_id, $area_id, $area_city);
	$best_goods = get_recommend_goods('best', '', $region_id, $area_id, $area_city);
	...
	if ($favourable['act_type'] == FAT_GOODS) {
		...
		$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
		$cart_goods = cart_goods($flow_type, $result['cart_value'], 0, $region_id, $area_id, $area_city);
				...
				if (empty($val['is_invalid'])) {
					...
					if ($xiangouInfo['is_xiangou'] == 1 && $start_date < $nowTime && $nowTime < $end_date) {
						...
						if ($xiangouInfo['xiangou_num'] <= $orderGoods['goods_number']) {
							...
							$sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET goods_number = 0 WHERE rec_id = \'' . $val['rec_id'] . '\'';
							$GLOBALS['db']->query($sql);
							$result['error'] = 1;
							exit($json->encode($result));
						}
							...
							if ($xiangouInfo['is_xiangou'] == 1 && $xiangouInfo['xiangou_num'] < $orderGoods['goods_number'] + $val['goods_number']) {
								...
								$cart_Num = $xiangouInfo['xiangou_num'] - $orderGoods['goods_number'];
								$sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . (' SET goods_number = \'' . $cart_Num . '\' WHERE rec_id = \'') . $val['rec_id'] . '\'';
								$GLOBALS['db']->query($sql);
								$result['error'] = 1;
								$result['cart_Num'] = $cart_Num;
								$result['rec_id'] = $key;
...
else if ($_REQUEST['act'] == 'onlinepay_edit') {
