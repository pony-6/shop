...
if ($_REQUEST['act'] == 'single_check') {
	...
	if ($_REQUEST['check'] == 'allow') {
		...
		if (!empty($_REQUEST['integ'])) {
			log_account_change($_REQUEST[user_id], $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = floatval(0 - $_REQUEST['integ']), $change_desc = $_LANG['show_img_no_reduce_intergral']);
		}
