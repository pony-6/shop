...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash' || $_REQUEST['act'] == 'no_comment') {
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		$smarty->assign('seller_stages', $_CFG['seller_stages']);
		assign_query_info();
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				$other = array('ru_id' => $adminru['ru_id'], 'desc_name' => $desc_name, 'goods_desc' => $goods_desc);
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
					...
					if ($GLOBALS['_CFG']['review_goods'] == 0) {
						...
						$review_goods = $db->getOne($sql, true);
					...
					if ($is_insert) {
						...
						foreach ($notice_list as $key => $val) {
							...
							if ($shop_price <= $val['hopeDiscount'] && $val['cellphone'] && $_CFG['sms_price_notice'] == '1') {
								...
								$smsParams = array('user_name' => $user_info['user_name'], 'username' => $user_info['user_name'], 'goodsname' => sub_str($goods_name, 20), 'goodsprice' => $shop_price, 'mobile_phone' => $val['cellphone'], 'mobilephone' => $val['cellphone']);
					...
					if (!empty($goods_sn)) {
						...
						if (!empty($goods_sn)) {
							$sql = 'SELECT p.product_id FROM ' . $ecs->table('products') . ' AS p,' . $ecs->table('goods') . ' AS g ' . ('WHERE p.product_sn = \'' . $goods_sn . '\' AND p.goods_id = g.goods_id AND g.user_id = \'') . $adminru['ru_id'] . '\'';
					...
					if ($data['status'] != 0) {
						...
						else if (strpos($data['result']['grossweight'], $_LANG['gram']) !== false) {
