...
if ($_REQUEST['act'] == 'list') {
	...
	$package_goods_list = get_package_goods($act_id, $_SESSION['seller_id'], 1);
	$smarty->assign('package', $package);
		...
		if (!empty($goods_ids_arr)) {
			foreach ($goods_ids_arr as $v) {
				...
				if ($goods_count == 0 || $type == 1) {
					$sql = 'INSERT INTO ' . $ecs->table('package_goods') . ' (package_id, goods_id, goods_number, admin_id) ' . ('VALUES (\'' . $package_id . '\', \'') . $v . ('\', \'' . $number . '\', \'' . $_SESSION['seller_id'] . '\')');
					$db->query($sql, 'SILENT');
				}
			}
		}
