...
function services_list()
{
	...
	$sql = 'SELECT COUNT(s.id) FROM ' . $GLOBALS['ecs']->table('im_service') . ' s' . ' INNER JOIN ' . $GLOBALS['ecs']->table('admin_user') . ' a ON a.user_id = s.user_id  AND a.ru_id = ' . $adminru['ru_id'] . ' WHERE s.status = 1';
	$filter['record_count'] = $GLOBALS['db']->getOne($sql);
	$filter = page_and_size($filter);
	$list = array();
	$sql = 'SELECT s.* FROM ' . $GLOBALS['ecs']->table('im_service') . ' s' . ' INNER JOIN ' . $GLOBALS['ecs']->table('admin_user') . ' a ON a.user_id = s.user_id  AND a.ru_id = ' . $adminru['ru_id'] . ' WHERE s.status = 1';
...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('ur_here', $_LANG['services_list']);
	$admins = admin_list();
