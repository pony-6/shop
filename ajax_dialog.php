...
if ($_REQUEST['act'] == 'shipping_type') {
	...
	$cart_goods = cart_goods($flow_type, $_SESSION['cart_value'], 0, $warehouse_id, $area_id);
	...
	if (1 < count($id)) {
		...
		$record_count = $db->getAll('SELECT oi.order_id FROM ' . $ecs->table('order_info') . ' as oi' . ' left join ' . $ecs->table('order_goods') . ' as og on oi.order_id = og.order_id' . $left_join . (' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = \'' . $type . '\' AND oi.extension_code = \'auction\' ') . ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi_2 where oi_2.main_order_id = oi.order_id) = 0 ' . $where . ' group by oi.order_id');
		$record_count = count($record_count);
	}
	else {
		$user_id = $id[0];
	...
	if (!empty($rec_number)) {
		...
		$cart_goods_list = cart_goods($flow_type, $rec_number, 1, $warehouse_id, $area_id, $area_city);
	...
	if ($shipping_prompt) {
		...
		$cart_goods_list = cart_goods($flow_type, $shipping_prompt, 1, $warehouse_id, $area_id, $area_city);
		$cart_goods_list_new = cart_by_favourable($cart_goods_list);
		$GLOBALS['smarty']->assign('goods_list', $cart_goods_list_new);
		$GLOBALS['smarty']->assign('cart_value', $cart_value);
		$GLOBALS['smarty']->assign('store_seller', $store_seller);
	...
	$GLOBALS['smarty']->assign('area_city', $area_city);
	...
	$result['area'] = array('region_id' => $area->region_id, 'area_id' => $area->area_id, 'area_city' => $area->area_city, 'province_id' => $area->province_id, 'city_id' => $area->city_id, 'district_id' => $area->district_id, 'street_id' => $area->street_id, 'street_list' => $area->street_list);
	...
	if (!empty($seller_store)) {
		foreach ($seller_store as $k => $v) {
			if (is_spec($is_spec) == true) {
				$products = get_warehouse_id_attr_number($v['goods_id'], $spec_arr, $v['ru_id'], 0, 0, 0, '', $v['id']);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_stock') {
	...
	if ($rec_id) {
		...
		$store_id = !empty($_REQUEST['store_id']) ? intval($_REQUEST['store_id']) : 0;
		$store_seller = !empty($_REQUEST['store_seller']) ? $_REQUEST['store_seller'] : '';
		$cart_goods_list = cart_goods($flow_type, $rec_id, 1, $warehouse_id, $area_id, $area_city);
		$cart_goods_list_new = cart_by_favourable($cart_goods_list);
		$GLOBALS['smarty']->assign('goods_list', $cart_goods_list_new);
		$GLOBALS['smarty']->assign('cart_value', $cart_value);
		$GLOBALS['smarty']->assign('store_seller', $store_seller);
